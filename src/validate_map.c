...
/*   Updated: 2025/10/20 22:57:32 by rdcm             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_border(t_game *game)
{
	...
	while (x < game->cols)
	{
		...
		{
			ft_printf("Error: O mapa não esta cercado por paredes (superior/inferior).\n");
			return (0);
		}
		x++;
	}
	...
	while (y < game->rows - 1)
	{
		...
		{
			ft_printf("Error: O mapa não esta cercado por paredes (laterais).\n");
			return (0);
		}
		y++;
	}
...
void flood_fill(char **map, int y, int x, t_game *game)
{
    ...
    if (map[y][x] == '1' || map[y][x] == 'V')
        return;
    if (map[y][x] == 'C' || map[y][x] == 'E' || map[y][x] == '0' || map[y][x] == 'P')
		map[y][x] = 'V';  
    else
        return;
